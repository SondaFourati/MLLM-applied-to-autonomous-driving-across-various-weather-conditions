...
from simModel.Model3 import Model    #***************************************************change*************************
...
class VLMAgent:
    ...
    def makeDecision(self):
        """
        A function that makes a decision based on a prompt, measures the time it takes to make the decision, and returns various relevant data including the behavior,
        the decision message, prompt tokens, completion tokens, total tokens, and the time cost.
        """
        start = time.time()
        ...
        if match:
            ...
            raise ValueError('GPT-4o did not return a valid decision')
        return (
...
#"""
SYSTEM_PROMPT = """
You are GPT-4V(ision), a large multi-modal model trained by OpenAI. Now you act as a mature driving assistant, who can give accurate and correct advice for human drivers in complex urban driving scenarios. You'll receive input from multiple sensors, including the onboard camera, LiDAR data, and images from the front-right, front-left cameras. You'll need to make driving inferences and decisions based on the information from all these sources.

At each decision frame, you receive navigation information, a collection of actions, LiDAR data, a front-view image, a front-right image, a front-left image. You will perform scene description and reasoning based on the navigation information and all the provided inputs. Eventually, you will select the appropriate action output from the action set.
...
## Description
Your description of the front-view image, front-right image, front-left image and LiDAR Data.

## Reasoning
Reasoning based on the navigation information, front-view image, front-right image, front-left image and LiDAR data.

## Decision
One of the actions in the action set. (SHOULD BE exactly the same and no other words!)
"""
        ...
        while not model.tpEnd:
            ...
            if model.timeStep % 10 == 0:
                ...
                if model.tpStart and roadgraph:
                    ...
                    # get the image prompt: the left front, front, and right front,  the backnt, back, right back
                    ...
                    #*********************************************************Added**************************************
                    #back_img = images[-1].CAM_BACK
                    #back_left_img = images[-1].CAM_BACK_LEFT
                    #back_right_img = images[-1].CAM_BACK_RIGHT
                    #**********************************************************************************************
                    if isinstance(front_img, np.ndarray):
                     ...
                     #*********************************************************Added*************************************   
                        #gpt4v.addTextPrompt('The next three images are images captured by the left back, back, and right back cameras.\n')   
                        #gpt4v.addImageBase64(NPImageEncode(back_left_img))
                        #gpt4v.addImageBase64(NPImageEncode(back_img))
                        #gpt4v.addImageBase64(NPImageEncode(back_right_img))
                #*****************************************************************************************************************
                               
                        lidar_data = model.getLidarData()  # Assuming you have a method to get LiDAR data defined in Model3.py
                        print(lidar_data)
